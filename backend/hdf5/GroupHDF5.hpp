...
public:
    ...
    bool hasEntity(const nix::Identity &id) const ;

    std::shared_ptr<base::IEntity> getEntity(const nix::Identity &id) const;

    std::shared_ptr<base::IEntity> getEntity(ObjectType type, ndsize_t index) const;

    ndsize_t entityCount(ObjectType type) const;

    bool removeEntity(const nix::Identity &id);

    void addEntity(const nix::Identity &id);
    /**
